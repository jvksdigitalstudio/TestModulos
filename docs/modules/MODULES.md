...
- `Voice` sigue usando parámetros de envolvente ADSR FIJOS, y nada
  ...
  que los alimente.
- **Fase A.1 (corregido):** `PitchQuality` (Fast/HighQuality) existía
  como enum en el contrato desde antes, pero nada lo consumía —
  `Voice::renderInto` tenía `dsp::InterpolationMethod::Cubic`
  hardcodeado como literal. Ahora `Voice::setPitchQuality`,
  `VoiceManager::setPitchQuality` y `SamplerEngine::setPitchQuality`
  están cableados de punta a punta (mismo patrón atómico que
  `setFilterCutoff`), verificado con un test que mide que `Fast` y
  `HighQuality` producen audio real y mediblemente distinto para una
  nota con pitch fraccional (no solo que compila). Default:
  `HighQuality` — preserva exactamente el comportamiento previo a este
  fix para quien no llame al método nuevo.
