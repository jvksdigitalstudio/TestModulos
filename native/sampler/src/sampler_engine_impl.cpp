...
public:
    ...
    void setMasterVolume(float linearGain) noexcept override {
    ...
    void renderBlock(float* outputBuffer, std::size_t frameCount) noexcept override {
        ...
        float volume = masterVolumeTarget_.load(std::memory_order_relaxed);
        dsp::applyGain(outputBuffer, frameCount, volume);
    }
