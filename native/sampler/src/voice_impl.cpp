...
dsp::InterpolationMethod toInterpolationMethod(PitchQuality quality) noexcept {
    switch (quality) {
        case PitchQuality::Fast:
            return dsp::InterpolationMethod::Linear;
        case PitchQuality::HighQuality:
            return dsp::InterpolationMethod::Cubic;
    }
    return dsp::InterpolationMethod::Cubic;  // no alcanzable si el enum no cambia
}
...
public:
    ...
    void setPitchQuality(PitchQuality quality) noexcept override {
        interpolationMethod_ = toInterpolationMethod(quality);
    }
...
private:
    void renderInto(float* buffer, std::size_t frameCount, bool accumulate) noexcept {
        for (std::size_t i = 0; i < frameCount; ++i) {
            ...
            float raw =
                dsp::interpolateSample(interpolationMethod_, y0, y1, y2, y3, frac);
    ...
    // Default HighQuality/Cubic: preserva exactamente el comportamiento
    // previo a que setPitchQuality() existiera (antes era un literal
    // hardcodeado dsp::InterpolationMethod::Cubic en renderInto).
    dsp::InterpolationMethod interpolationMethod_{dsp::InterpolationMethod::Cubic};
};
