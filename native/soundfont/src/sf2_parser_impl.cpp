...
public:
    // NO noexcept (fix §9): el cuerpo de parseImpl() hace decenas de
    // asignaciones dinámicas (std::string, std::vector::push_back,
    // reserve, move) para construir el SoundFontModel completo. Es
    // una ruta OFFLINE, no realtime — permitir una excepción real
    // (p.ej. std::bad_alloc bajo memoria agotada de verdad) es
    // aceptable; lo que NO es aceptable es que esa excepción escape
    // de una función noexcept y dispare std::terminate() sin ninguna
    // posibilidad de que el llamador la maneje. Antes, `parse()`
    // estaba marcado noexcept sin que esa garantía fuera real —
    // exactamente el caso que Fase A.1 §9 pide auditar y corregir.
    // Política explícita: cualquier excepción se atrapa aquí y se
    // convierte en un Result<SoundFontModel>::fail() normal.
    Result<SoundFontModel> parse(const ByteSpan& data) override {
        try {
            return parseImpl(data);
        } catch (const std::bad_alloc&) {
            return fail(ErrorCode::OutOfMemory,
                        "Sin memoria suficiente para parsear este SoundFont "
                        "(archivo posiblemente corrupto con tamaños de chunk "
                        "absurdamente grandes, o memoria real agotada).");
        } catch (const std::exception& e) {
            return fail(ErrorCode::Unknown,
                        std::string("Excepción inesperada durante el parseo: ") + e.what());
        }
    }

private:
    Result<SoundFontModel> parseImpl(const ByteSpan& data) {
        SafeByteReader reader(data);
