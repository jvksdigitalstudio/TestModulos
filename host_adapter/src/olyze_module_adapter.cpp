...
OlyzeResult moduleSetParameter(OlyzeModuleHandle handle, OlyzeParameterChange change) {
    ...
    // rampMs se ignora deliberadamente en esta fase: setMasterVolume/
    // setFilterCutoff aplican el valor de inmediato en el siguiente
    // bloque (vía std::atomic, ver sampler_engine_impl.cpp) — sin
    // rampa suave todavía. Cambios bruscos de volumen/filtro pueden
    // producir un "zipper" audible; documentado como limitación, no
    // fingido como resuelto.
    switch (change.parameterId) {
        case kParamMasterVolume:
            inst->engine->setMasterVolume(clamped);
            break;
